...
void Wheel::calculateSlipRatio(
    ...
    // slip ratio (коэффициент скольжени€) - соотношение угловой скорости колеса к линейной
    ...
    if (isBrakingByWheelsOrEngine) {
        ...
        // сила торможени€ направлена противоположно скорости
        if (Numeric::getSign(linearVelocityProjection) == Numeric::getSign(slipRatio)) {
            slipRatio = -slipRatio;
        }
    } else {
        // сила разгона направлена по ходу движени€
        if (gear >= Gear::first) {
...
void Wheel::calculateSlipAngle(Vector3 vehicleLinearVelocity) {
    // slip angle (угол увода) - угол между направлением, в которое повернуто колесо, и направлением его движени€
    if (vehicleLinearVelocity.isZero() || Numeric::floatEquals(_angularVelocity, 0.0f, VehicleConstants::minLinearVelocityDelta)) {
        _slipAngle = 0.0f;
        return;
    }
    // знак lateralVelocity разный дл€ левого и правого колеса
    // longitudinalVelocity всегда положительный, дл€ slip angle не важно едет колесо вперед или назад
...
void Wheel::normalizeLongitudinalAndLateralForces(float springForce) {
    // friction circle (Kamm's circle) круг сцеплени€ (диаграмма  амма)
    // на самом деле это эллипс, тк максимальные значени€ продольной (longitudinal) и поперечной (lateral) сил не равны друг другу
    ...
    // находим любую точку пересечени€ пр€мой и эллипса
    // подробности в 'docs\friction circle.jpg'
    float k = curLong / curLater; // угловой коэфф пр€мой (вектора суммы сил)
...
void Wheel::calculateRoadFrictionForce(Vector3 vehicleLinearVelocity) {
    // дл€ вычислени€ силы трени€ врем€ dt не используем
    // тк эта сила вычисл€етс€ на основе скорости, котора€ уже посчитана с учетом dt
